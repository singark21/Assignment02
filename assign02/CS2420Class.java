 ...
 * 
 * @author Erin Parker and Arkein Singh and Brayden Newsom
...
public class CS2420Class {
	...
	public ArrayList<CS2420Student> lookup(EmailAddress contactInfo) {
		...
		for(int pos = 0; pos < studentList.size(); pos++) {
			if(studentList.get(pos).getContactInfo().equals(contactInfo))
				sameContactInfo.add(studentList.get(pos));
		}
	...
	public double computeClassAverage() {
		...
		for(int pos = 0; pos < studentList.size(); pos++) {
			averageScore += studentList.get(pos).computeFinalScore();
		}
	...
	public ArrayList<EmailAddress> getContactList() {
		ArrayList<EmailAddress> studentsContactInfo = new ArrayList<>();
		for(int pos = 0; pos < studentList.size(); pos++) {
			if(!studentsContactInfo.contains(studentList.get(pos).getContactInfo()))
			studentsContactInfo.add(studentList.get(pos).getContactInfo());
		}
		return studentsContactInfo;
	}
